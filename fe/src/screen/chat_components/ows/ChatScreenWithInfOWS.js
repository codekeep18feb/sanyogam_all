...
const ChatScreenHeader = forwardRef(
  (
    {
      setvideoView,
      videoView,
...
const ChatScreenWithInfOWS = forwardRef(
  (
    {
      soc_conn,
      videoView,
    ...
    const renderContent = () => {
      ...
      if (requestStatus === "ACCEPTED" && connection_open) {
        ...
        return (
          <div>
            Opening the connection...- Mode - {JSON.stringify(videoView)}
          </div>
        );
      } else if (videoView) {
        return (
          <div>
            <div>
              we will now render the VideoWindow which should appranetly do
              everything make sure this is write and for opponent -{with_userid}
            </div>
        ...
        return (
          <div>
            It seems connection was closed from either side -{" "}
            {JSON.stringify(videoView)}
          </div>
        );
      } else {
    ...
    return (
      <div>
        ...
        <div>soc_conn here - {soc_conn}</div>
        {renderContent()}
      </div>
    );
  }
