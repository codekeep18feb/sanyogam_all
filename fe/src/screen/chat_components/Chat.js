...
function Chat({ auth_data }) {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        "http://192.168.1.4:8000/api/read_online_circle",
        {
          method: "GET",
          headers: {
  ...
  //   try {
  //     const response = await fetch(`http://192.168.1.4:8000/api/rtc_user_info_by_id`, {
  //       method: 'GET',
  //       headers: {
  //         'Content-Type': 'application/json',
  //         'Authorization': token,
  ...
  useEffect(async () => {
    ...
    const response2 = await fetch("http://192.168.1.4:8000/api/profiles", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
