...
export default function ChatWindowWS({ SetWithUserId, SetWithEmail, with_email, with_userid }) {
  ...
  React.useEffect(() => {
    socket.on('message', (data) => {
      console.log('tyepfof',data,typeof(data))
      ...
      if (data){
        ...
        console.log('whatisin resp case')
  ...
  const sendMessage = (message) => {
    ...
    socket.emit('write_rtc_pool', message);
    // setMessageInput('');
  };
  ...
  const fetchRTCUserInfo = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("Error fetching fetchRTCUserInfo");
      }
    } catch (error) {
      console.error("An error occurred:", error);
    } finally {
  ...
  const addRTCUserInfo = async (sdp, to_user) => {
    console.log("hereisto_user", sdp, to_user);
    const JWT_TOKEN = localStorage.getItem("token");
    const token = `Bearer ${JWT_TOKEN}`;
    const payload = {
    ...
    try {
      ...
      console.log('what is the type of sdp',sdp,typeof(sdp))
      ...
      if (response.status === 200) {
        ...
        console.log("Error fetching addRTCUserInfo");
      }
    } catch (error) {
      console.error("An error occurred:", error);
    } finally {
  ...
  const fetchRTCOffer = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("Error fetching fetchRTCOffer");
      }
    } catch (error) {
      console.error("An error occurred:", error);
    } finally {
  ...
  const fetchUserId = async (token, with_email) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.log("Error fetching fetchUserId");
        return null;
      }
    } catch (error) {
      console.error("An error occurred:", error);
    ...
    if (type == "INITIATOR") {
      ...
      lc.onicecandidate = async (e) => {
        ...
        if (e.candidate) {
          ...
          addRTCUserInfo(JSON.stringify(lc.localDescription), to_user_id);
        }
      };
    ...
    >
      {(requestStatus) ? (
        <div>
          ...
          <ChatScreenWithInfoWS ref={myRef} setvideoView={setvideoView} videoView={videoView} with_userid={with_userid} SetWithUserId={SetWithUserId} SetWithEmail={SetWithEmail} requestStatus={requestStatus} connection_open={connection_open} with_email={with_email} chats={chats} sendMsg={sendMsg}/>
        </div>
