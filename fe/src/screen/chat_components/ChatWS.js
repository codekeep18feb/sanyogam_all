...
function ChatWS({ auth_data }) {
  ...
  // const [socket, setSocket] = useState(io.connect('http://192.168.1.8:8000'));
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(
        "http://192.168.1.8:8000/api/read_online_circle",
        {
          method: "GET",
          headers: {
  ...
  const fetchRTCUserInfo = async () => {
    ...
    try {
      const response = await fetch(
        `http://192.168.1.8:8000/api/rtc_user_info_by_id`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
