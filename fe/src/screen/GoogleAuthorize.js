...
export function GoogleAuthorize({login}) {
  ...
  const GOOGLE_REDIRECT_URI = 'http://54.234.122.52:3000/google_authorized';
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://54.234.122.52:8000/api/save_user_detail', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.status === 201) {
        ...
        try {
          const response = await fetch('http://54.234.122.52:8000/api/login', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
          ...
          if (response.status === 201) {
            ...
            if (data){
              const response = await fetch('http://54.234.122.52:8000/api/me', {
              method: 'GET',
              headers: {
                'Content-Type': 'application/json',
