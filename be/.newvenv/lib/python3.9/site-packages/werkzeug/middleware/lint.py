...
class LintMiddleware:
    ...
    def check_start_response(
        ...
        if type(headers) is not list:
            warn("HeaderDesktop list is not a list.", WSGIWarning, stacklevel=3)

        for item in headers:
            if type(item) is not tuple or len(item) != 2:
                warn("HeaderDesktop items must be 2-item tuples.", WSGIWarning, stacklevel=3)
            name, value = item
            if type(name) is not str or type(value) is not str:
                warn(
                    "HeaderDesktop keys and values must be strings.", WSGIWarning, stacklevel=3
                )
